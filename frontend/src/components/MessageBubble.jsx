...
export default function MessageBubble({ message, animeMetadata }) {
  ...
  return (
    <div className={`flex gap-4 w-full mb-6 max-w-full ${isUser ? 'flex-row-reverse self-end justify-start' : 'flex-row self-start'}`}>
      
      {/* Avatar Icon */}
      <div className={`
        w-[38px] h-[38px] rounded-lg flex items-center justify-center shrink-0 bg-bg-secondary border shadow-sm transition-transform duration-200
        ${isUser 
          ? 'border-white/10' 
          : 'border-crimson/20 shadow-[0_0_10px_rgba(190,18,60,0.15)]'
        }
      `}>
        {isUser ? (
          // Custom User Icon
          <svg width="24" height="24" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M50 10 L80 25 L80 60 C80 75 50 90 50 90 C50 90 20 75 20 60 L20 25 Z" stroke="var(--text-secondary, #cbd5e1)" strokeWidth="6" strokeLinejoin="round" fill="rgba(148, 163, 184, 0.15)"/>
            <circle cx="50" cy="40" r="14" stroke="var(--text-secondary, #cbd5e1)" strokeWidth="4"/>
            <path d="M28 72 C28 60 38 52 50 52 C62 52 72 60 72 72" stroke="var(--text-secondary, #cbd5e1)" strokeWidth="4" strokeLinecap="round"/>
          </svg>
        ) : (
          // Bot Hexagon Logo Icon
          <svg width="24" height="24" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className={`
        ...
        ${isUser 
          ? 'bg-bg-message-user text-text-primary border-none self-end' 
          : 'bg-bg-message-bot text-text-secondary border border-crimson/20 border-l-[3px] border-l-crimson self-start'
        }
      `}>
        {isUser ? (
